...
if __name__ == "__main__":
    ...
    constrains_name = ['take off', 'stall speed', 'cruise', 'service ceiling', 'level turn @3000m',
                       'climb @S-L', 'climb @3000m', 'climb @7000m', 'feasible region']
    ...
    for k in range(8):
        for i in range(m):
            ...
            if i != 1:
                ax[k].plot(w_s, p_w[i, :, k], color=color[i],
                           linewidth=1.1, label=constrains_name[i])
            elif i == 1 and k == 6:
                ax[k].plot(p_w[i, :, k], np.linspace(
                    0, 250, n), color=color[i], linewidth=1.1, label=constrains_name[i])
            elif i == 1 and k ==7:
                ax[k].plot(p_w[i, :, k], np.linspace(
                    0, 250, n), color=color[i], linewidth=1.1, label=constrains_name[i])
            else:
        ...
        if k == 6 or k == 7:
            p_w[1, :, k] = 200 / (p_w[1, -1, k] - p_w[1, 20, k]) * (w_s - p_w[1, 2, k])ax[k].fill_between(w_s, np.amax(p_w[0:m, :, k], axis=0),
                           200, color='b', alpha=0.25, label=constrains_name[-1])
        ...
        ax[k].set_ylim(0, 125)
        # plt.tight_layout()
        #ax[k].set_title(subtitle[k])
        ax[k].grid()
    #plt.gca().get_legend_handles_labels()
    handles, labels = plt.gca().get_legend_handles_labels()
    fig.legend(handles, labels, bbox_to_anchor=(0.1, 0.01, 0.8, 0.25), loc="lower left",
               mode="expand", borderaxespad=0, ncol=5)
    plt.tight_layout
    plt.setp(ax[0].set_title(r'$\bf{Mattingly-Method}$'))
    plt.setp(ax[1].set_title(r'$\bf{Gudmundsson-Method}$'))
    plt.setp(ax[6:7], xlabel='Wing Load: $W_{TO}$/S (N/${m^2}$)')
    plt.setp(ax[0], ylabel=r'$\bf{Turbofun}$''\n $P_{SL}$/$W_{TO}$ (W/N)')
    plt.setp(ax[2], ylabel=r'$\bf{Motor}$ ''\n $P_{SL}$/$W_{TO}$ (W/N)')
    plt.setp(ax[4], ylabel=r'$\bf{Total}$ ''\n $P_{SL}$/$W_{TO}$ (W/N)')
    plt.setp(ax[6], ylabel=r'$\bf{Conventional}$ ''\n $P_{SL}$/$W_{TO}$ (W/N)')
    plt.show()
