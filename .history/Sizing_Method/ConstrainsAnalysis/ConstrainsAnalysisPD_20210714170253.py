...
if __name__ == "__main__":
    ...
    for k in range(3):
        ...
        p_w[1, :] = 10 ** 10 * (w_s - p_w[0, 0])
        p_w[1 + m, :] = 10 ** 10 * (w_s - p_w[m, 0])
        plt.fill_between(w_s, np.amax(
            p_w[0:m-1, :], axis=0), 200, color='b', alpha=0.25, label=label[k])
        plt.fill_between(w_s, np.amax(p_w[m:2 * m-1, :], axis=0), 200, color='r', alpha=0.25, label=label[k + 3])
