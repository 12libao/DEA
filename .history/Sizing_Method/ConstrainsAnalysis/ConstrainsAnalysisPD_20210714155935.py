...
if __name__ == "__main__":
    ...
    for k in range(3):
        ...
        if k != 2:
            p_w[1+m, :] = 10 ** 10 * (w_s - p_w[1+m, 0]-10)
        else:
            def func(x, a0, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10):
                f = a0 + a1*x + a2*x**2 + a3*x**3 + a4*x**4 + a5 * \
                    np.sin(x) + a6*np.cos(x) + a7*x**5 + \
            ...
            popt, _ = curve_fit(func, xdata, ydata)
            p_w[1+m, :] = func(w_s, popt[0], popt[1], popt[2],
                             popt[3], popt[4], popt[5], popt[6], popt[7], popt[8], popt[9], popt[10])
        w_s = np.linspace(100, p_w[1, 1], n)
        plt.fill_between(w_s, np.amax(p_w[0, :], p_w[2:m, :], axis=0), 200, color='b', alpha=0.25,
                         label=label[k])
        w_s = np.linspace(100, p_w[1+m, 1], n)
        plt.fill_between(w_s, np.amax(p_w[m, :], p_w[2:m, :], p_w[m:2 * m, :], axis=0), 200, color='r', alpha=0.25,
                         label=label[k + 3])
