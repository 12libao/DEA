        ...
        for i in range(m):
            for j in range(n):
                ...
                if k < 4:
                    ...
                    if i == 1:
                        p_w[i, j, 4] = ConstrainsAnalysis_Mattingly_Method_with_DP_turbofun(
                            h, v, beta, w_s[j], hp).allFuncs[i](problem)
                        ax[k].plot(p_w[i, :, k], np.linspace(
                            0, 250, n), color=color[i], label=constrains_name[i])
                    else:
                        p_w[i, j, 4] = p_w[i, j, 0] + p_w[i, j, 2]
                else:
                    if i == 1:
                        p_w[i, j, 5] = ConstrainsAnalysis_Gudmundsson_Method_with_DP_turbofun(
                            h, v, beta, w_s[j], hp).allFuncs[i](problem)
                    else:
                        p_w[i, j, 5] = p_w[i, j, 1] + p_w[i, j, 3]
