...
class Design_Point_Select_Strategy:
    ...
    def strategy(self):
        if self.strategy_apply == 0:
            ...
            p_w_1_min, p_w_2_min = Design_Point_Select_Strategy.p_w_min(self, p_w)

            if self.propulsion_constrains == 0:
                p_w_turbofan_max = 100000
                p_w_motorfun_max = 100000
            else:
                p_w_turbofan_max = math.ceil(p_w_1_min)
                p_w_motorfun_max = math.ceil(p_w_2_min)

                # build p_w_design_point matrix, try p_w_max to find the best one
                p_w_design_point = np.zeros([p_w_turbofan_max+1, p_w_motorfun_max+1])
                for i in range(p_w_turbofan_max+1):
                    for j in range(p_w_motorfun_max+1):
            ...
            p_w_1_min, p_w_2_min = Design_Point_Select_Strategy.p_w_min(
                self, p_w)
            ...
            p_w_1_min = np.amax(p_w_1_min)
            p_w_2_min = np.amax(p_w_2_min)
...
if __name__ == "__main__":
    ...
    problem = Design_Point_Select_Strategy(h, v, beta, method=2, strategy_apply=1, propulsion_constrains=0)
