...
if __name__ == "__main__":
    ...
    for k in range(8):
        for i in range(m):
            ...
            if i == 1:
                ax[k].plot(p_w[i, :, k], np.linspace(0, 250, n), color=color[i], linewidth=2, label=constrains_name[i])
            else:
                ax[k].plot(w_s, p_w[i, :, k], color=color[i], linewidth=1, label=constrains_name[i])
