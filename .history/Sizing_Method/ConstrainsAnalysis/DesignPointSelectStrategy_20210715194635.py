...
if __name__ == "__main__":
    ...
    for z in range(3):
        ...
        design_point_w_s[z, 0], design_point_p_w[z, 4], design_point_p_w[z, 0], design_point_p_w[z, 2], hp_p_w_min[z, :], _, _ = problem1.strategy()
        design_point_w_s[z, 0], design_point_p_w[z, 5], design_point_p_w[z, 1], design_point_p_w[z, 3], hp_p_w_min[z+1, :], _, _ = problem2.strategy()
        ...
        for k in range(6):
            for i in range(m):
                ...
                if i == 0:
                    ax[k].plot(p_w[i, :, k], np.linspace(0, 100, n), 
                                linewidth=1, alpha=0.25, linestyle=l_style[z], label=constrains_name[i])
                else:
            ...
            ax[k].fill_between(w_s, np.amax(p_w[0:m, :, k], axis=0), 150, color=color2[z], alpha=0.5, label=l1[z])
            ax[k].plot(design_point_w_s[z, 0], design_point_p_w[z, k], marker=mark[z], color=color2[z],
