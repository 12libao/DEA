...
class Design_Point_Select_Strategy:
    ...
    def __init__(self, altitude, velocity, beta, method, p_w_turbofan_max=20, p_w_motorfun_max=24, n=12):
        """
    ...
    def strategy(self):
        ...
        p_w_design_point = np.zeros([100, 100])
        for i in range(50):
            for j in range(50):
        ...
        p_w_turbofan_max = np.unravel_index(
            p_w_design_point.argmax(), p_w_design_point.shape)[0]
        p_w_motorfun_max = np.unravel_index(
            p_w_design_point.argmax(), p_w_design_point.shape)[1]
