    ...
    def strategy(self):
        if self.strategy_apply == 0:
            ...
            p_w, p_w_1, p_w_2 = Design_Point_Select_Strategy.p_w_compute(
                self, p_w_turbofan_max, p_w_motorfun_max)
            #find the min p_w for difference hp for each flight condition:
            p_w_min = np.amin(p_w, axis=0)
            p_w_1_min = np.amax(np.amin(p_w_1, axis=0))
            p_w_2_min = np.amax(np.amin(p_w_2, axis=0))
...
if __name__ == "__main__":
    ...
    # plots
    fig, ax = plt.subplots(3, 2, sharey=True, sharex=True, figsize=(12, 10))
    ax = ax.flatten()
    for k in range(8):
        for i in range(m):
            for j in range(n):
