...
if __name__ == "__main__":
    ...
    for k in range(8):
        ...
        if k <= 5:
            ax[k].fill_between(w_s, np.amax(p_w[0:m, :, k], axis=0),
                               200, color='b', alpha=0.25, label=constrains_name[-2])
            ax[k].set_xlim(200, 9000)
            ax[k].set_ylim(0, 125)
            ax[k].grid()
        else:
            p_w[1, :, k] = 200 / \
                (p_w[1, -1, k] - p_w[1, 20, k]) * (w_s - p_w[1, 2, k])
            ax[k-2].fill_between(w_s, np.amax(p_w[0:m, :, k], axis=0),
                                 200, color='r', alpha=0.25, label=constrains_name[-1])
            ax[k-2].set_xlim(200, 9000)
            ax[k-2].set_ylim(0, 125)
            ax[k-2].grid()
    ...
    plt.setp(ax[4:5], xlabel='Wing Load: $W_{TO}$/S (N/${m^2}$)')
    plt.setp(ax[0], ylabel=r'$\bf{Turbofun}$''\n $P_{SL}$/$W_{TO}$ (W/N)')
    plt.setp(ax[2], ylabel=r'$\bf{Motor}$ ''\n $P_{SL}$/$W_{TO}$ (W/N)')
    plt.setp(ax[4], ylabel=r'$\bf{Total vs Conventional}$ ''\n $P_{SL}$/$W_{TO}$ (W/N)')
    plt.show()
