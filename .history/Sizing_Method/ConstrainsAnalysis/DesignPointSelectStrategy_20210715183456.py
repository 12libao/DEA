...
if __name__ == "__main__":
    ...
    for z in range(3):
        ...
        w_s, p_w_min, p_w_1_min, p_w_2_min, hp_p_w_min, p_w_turbofan_max, p_w_motorfun_max = problem.strategy()
        ...
        for k in range(6):
            for i in range(m):
                ...
                # plot the lines
                if i == 0:
                    ax[k].plot(p_w[i, :, k], np.linspace(0, 100, n), linewidth=1, alpha=0.5, linestyle=l_style[z], label=constrains_name[i])
                else:
