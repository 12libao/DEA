...
class Design_Point_Select_Strategy:
    ...
    def func_for_curve_fiting(x, a0, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10):
        f = a0 + a1*x + a2*x**2 + a3*x**3 + a4*x**4 + a5 * \
            np.sin(x) + a6*np.cos(x) + a7*x**5 + a8*x**6 + a9*x**7 + a10*x**8
        return f


    def p_w_stall_speed(self, method):
        for i in range(self.n):
            problem = method(self.h[0], self.v[0], self.beta[0], self.w_s[i], self.hp_threshold)
            self.p_w[0, i] = problem.allFuncs[1](problem)

        xdata, ydata = p_w[1, :], np.linspace(0, 250, self.n)
