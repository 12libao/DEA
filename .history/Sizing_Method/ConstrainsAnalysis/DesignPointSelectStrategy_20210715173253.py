    ...
    def strategy(self):
        if self.strategy_apply == 0:
            ...
            if self.propulsion_constrains == 0:
                ...
                p_w, _, _ = Design_Point_Select_Strategy.p_w_compute(self, p_w_turbofan_max, p_w_motorfun_max, 0)
            else:
                p_w, _, _ = Design_Point_Select_Strategy.p_w_compute(
                    self, 10000, 10000, pc=0)
            p_w_1_min, p_w_2_min, _, _ = Design_Point_Select_Strategy.p_w_min(
                self, p_w)
                p_w_turbofan_max = math.ceil(p_w_1_min)
                p_w_motorfun_max = math.ceil(p_w_2_min)

                ic(p_w_turbofan_max, p_w_motorfun_max)
