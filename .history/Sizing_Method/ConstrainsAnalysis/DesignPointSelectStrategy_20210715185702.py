...
if __name__ == "__main__":
    ...
    design_point_p_w design_point_w_s = np.zeros([3, 6]), np.zeros([3, 6])
    for z in range(3):
        h = constrains[:, 0]
        v = constrains[:, 1]
        beta = constrains[:, 2]
        ...
        design_point, p_w_min[z*2], p_w_1_min_M, p_w_2_min_M, hp_p_w_min_M, p_w_turbofan_max_M, p_w_motorfun_max_M = problem1.strategy()
        w_s[z*2+1], p_w_min[z*2+1], p_w_1_min_G, p_w_2_min_G, hp_p_w_min_G, p_w_turbofan_max_G, p_w_motorfun_max_G = problem1.strategy()
