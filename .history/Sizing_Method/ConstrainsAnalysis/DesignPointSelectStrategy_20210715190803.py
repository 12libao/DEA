...
if __name__ == "__main__":
    ...
    design_point_p_w, design_point_w_s, hp_p_w_min = np.zeros([3, 6]), np.zeros([3, 2]), np.zeros([2, 3, 8])
    hp_p_w_min = np.zeros([2, 3, 8])
    for z in range(3):
        h = constrains[:, 0]
        v = constrains[:, 1]
        beta = constrains[:, 2]
        ...
        design_point_p_w[z, 0], design_point_p_w[z, 2], hp_p_w_min[1, z, :], _, _ = problem1.strategy()
        design_point_w_s[z, 0], design_point_p_w[z, 5], 
        design_point_p_w[z, 1], design_point_p_w[z, 3], hp_p_w_min[2, z, :], _, _ = problem1.strategy()
