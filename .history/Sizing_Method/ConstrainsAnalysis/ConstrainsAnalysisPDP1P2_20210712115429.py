...
if __name__ == "__main__":
    ...
    for k in range(8):
        for i in range(m):
            ...
            if i == 1:
                axes[k].plot(p_w[i, :], np.linspace(0, 250, n), color=color[i], label=constrains_name[i])
            else:
                axes[k].plot(w_s, p_w[i, :], color=color[i], label=constrains_name[i])

        p_w[1, :] = 200 / (p_w[1, -1] - p_w[1, 20]) * (w_s - p_w[1, 2])
        axes[k].fill_between(w_s, np.amax(p_w[0:m - 1, :], axis=0), 200, color='b', alpha=0.25, label=label[k])

        # plt.xlabel('Wing Load: $W_{TO}$/S (N/${m^2}$)')
        # plt.ylabel('Power-to-Load: $P_{SL}$/$W_{TO}$ (W/N)')
        axes[k].legend(bbox_to_anchor=(1.002, 1), loc="upper left")
