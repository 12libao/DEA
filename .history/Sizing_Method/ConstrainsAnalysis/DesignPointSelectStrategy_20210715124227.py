...
class Design_Point_Select_Strategy:
    ...
    def __init__(self, altitude, velocity, beta, method=1, strategy_apply=0, propulsion_constrains=0, n=12):
        """
        ...
        :param method: if method = 0, it is Mattingly Method, otherwise is Gudmundsson Method
        ...
        # method = 0 = Mattingly_Method, method = 1 = Gudmundsson_Method
        if method == 0:
    ...
    def p_w_compute(self, p_w_turbofan_max, p_w_motorfun_max):
    ...
    def strategy(self):
        if self.strategy_apply == 0:
            ...
            if self.propulsion_constrains == 0:
                ...
                p_w_design_point = np.zeros([50, 40])
                for i in range(60):
                    for j in range(50):
                        p_w, _, _ = Design_Point_Select_Strategy.p_w_compute(self, i, j)
            ...
            p_w, _, p_w_2 = Design_Point_Select_Strategy.p_w_compute(
                self, p_w_turbofan_max, p_w_motorfun_max)
            
            ic(p_w)
            #find the min p_w for difference hp for each flight condition:
            p_w_min = np.amin(p_w, axis=1)
            p_w_1_min = np.amax(np.amin(p_w_1[:, 1:-2], axis=1))
            p_w_2_min = np.amax(np.amin(p_w_2[:, 1:-2], axis=1))
            ic(p_w_1, p_w_2)
            ...
            p_w_1_min = np.zeros(8)
            p_w_2_min = np.zeros(8)
            for i in range(1,8):
                ...
                problem2 = self.method2(
                    self.h[i], self.v[i], self.beta[i], self.w_s, hp_p_w_min[i])
                if i >= 5:
                    p_w_1_min[i] = problem1.allFuncs[-1](problem1, roc=15 - 5 * (i - 5))
                    p_w_2_min[i] = problem2.allFuncs[-1](problem2, roc=15 - 5 * (i - 5))
                else:
                    p_w_1_min[i] = problem1.allFuncs[i](problem1)
                    p_w_2_min[i] = problem2.allFuncs[i](problem2)
        ...
        return hp_p_w_min, design_point, p_w_1_min, p_w_2_min, p_w_turbofan_max, p_w_motorfun_max
...
if __name__ == "__main__":
    ...
    problem = Design_Point_Select_Strategy(h, v, beta, method=1, strategy_apply=1, propulsion_constrains=0)
    hp_p_w_min, design_point, p_w_1_min, p_w_2_min, p_w_turbofan_max, p_w_motorfun_max = problem.strategy()
    ic(hp_p_w_min, design_point, p_w_1_min, p_w_2_min, p_w_turbofan_max, p_w_motorfun_max)
