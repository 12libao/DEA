   ...
   def __init__(self, altitude, velocity, beta, wing_load, Hp=0.2, number_of_motor=12, C_DR=0):
    ...
    def master_equation(self, n, dh_dt, dV_dt):
        ...
        p_w = self.coefficient * \
           (self.q / (self.beta * self.w_s) * cd + dh_dt / self.v + dV_dt / self.g0)
        return p_w

    def cruise(self):
        p_w = ConstrainsAnalysis_Mattingly_Method_with_DP.master_equation(
...
if __name__ == "__main__":
    ...
    for k in range(8):
        ...
        ax[k].fill_between(w_s, np.amax(p_w[0:m, :, k], axis=0),
                           200, color='b', alpha=0.25, label=label[k])
