...
if __name__ == "__main__":
    ...
    # plots 
    fig, ax = plt.subplots(4, 2, sharex=True, sharey=True, figsize=(15, 10))
    ax = ax.flatten()
    for k in range(8):
        for i in range(m):
            for j in range(n):
        ...
        ax[k].fill_between(w_s, np.amax(p_w[0:m, :, k], axis=0), 200, color='b', alpha=0.25)
    ...
    plt.setp(ax[0], ylabel='Turbofun Power-to-Load: $P_{SL}$/$W_{TO}$ (W/N)')
    plt.setp(ax[2], ylabel='Motor Power-to-Load: $P_{SL}$/$W_{TO}$ (W/N)')
    plt.setp(ax[4], ylabel='Total Power-to-Load: $P_{SL}$/$W_{TO}$ (W/N)')
    plt.setp(ax[6], ylabel='Conventional Power-to-Load: $P_{SL}$/$W_{TO}$ (W/N)')
    plt.show()
