...
if __name__ == "__main__":
    ...
    lablel1 = ['hp: 0.5', 'hp: adjust', 'hp: adjust with threshold']
    lablel2 = ['design point hp: 0.5', 'design point hp: adjust', 'design point hp: adjust with threshold']
    ...
    for z in range(3):
        ...
        for k in range(6):
            ...
            ax[k].fill_between(w_s, np.amax(p_w[0:m, :, k], axis=0), 150, color=color2[z], alpha=0.5, label=constrains_name[-2])
            ax[k].plot(design_point_w_s[z, 0], design_point_p_w[z, k], marker=mark[z], color=color2[z],
                       markersize=5, label='True Conventional')
