    ...
    def strategy(self):
        if self.strategy_apply == 0:
            ...
            if self.propulsion_constrains == 0:
                ...
                p_w_turbofan_max = math.ceil(p_w_1_min) + 1
                p_w_motorfun_max = math.ceil(p_w_2_min) + 1

                # build p_w_design_point matrix, try p_w_max to find the best one
                p_w_design_point = np.zeros([p_w_turbofan_max, p_w_motorfun_max])
                for i in range(p_w_turbofan_max):
