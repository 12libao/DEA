...
class Design_Point_Select_Strategy:
    ...
    def __init__(self, altitude, velocity, beta, method, p_turbofan_max, p_motorfun_max, n=12):
        ...
        self.p_w = np.zeros(self.m)  # m x n matrix

        self.hp = np.linspace(0, 1, self.n)
        self.hp_threshold = self.p_motorfun_max / (self.p_motorfun_max + self.p_turbofan_max)

        if method == Mattingly:
            self.method1 = ca_pd_12ConstrainsAnalysis_Mattingly_Method_with_DP_turbofun
            self.method2 = ConstrainsAnalysis_Mattingly_Method_with_DP_electric
    ...
    def strategy(self):
        for i in range(1, 8):
            for j in range(len(self.n)):
                problem = self.method(self.h[i], self.v[i],
                                    self.beta[i], self.w_s, self.hp[j])
                if i >= 5:
                    self.p_w[i] = problem.allFuncs[-1](problem, roc=15 - 5 * (i - 5))
                else:
                    self.p_w[i] = problem.allFuncs[i](problem)
