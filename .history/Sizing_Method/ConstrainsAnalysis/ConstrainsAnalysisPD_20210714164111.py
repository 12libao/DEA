...
if __name__ == "__main__":
    ...
    for k in range(3):
        ...
        plt.fill_between(w_s, np.amax(p_w[1:m-1, :], axis=0), 200, color='b', alpha=0.25, label=label[k])
        w_s = np.linspace(100, p_w[m, 1], n)
        plt.fill_between(w_s, np.amax(p_w[m+1:2 * m-1, :], axis=0), 200, color='r', alpha=0.25,
                         label=label[k + 3])
