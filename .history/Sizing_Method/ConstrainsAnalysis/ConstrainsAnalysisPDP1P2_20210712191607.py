...
if __name__ == "__main__":
    ...
    for k in range(8):
        for i in range(m):
            ...
            if i == 1 and k != 6 and k != 7:
                xdata, ydata = p_w[i, :], np.linspace(0, 250, n)
                popt, _ = curve_fit(func, xdata, ydata)
                p_w[i, :] = func(w_s, popt[0], popt[1], popt[2],
            ...
            if i ==1 and (k == 6 or k == 7):
                ax[k].plot(p_w[i, :, k], np.linspace(0, 250, n), color=color[i], linewidth=1.1, label=constrains_name[i])
            else:
                ax[k].plot(w_s, p_w[i, :, k], color=color[i], linewidth=1.1, label=constrains_name[i])
